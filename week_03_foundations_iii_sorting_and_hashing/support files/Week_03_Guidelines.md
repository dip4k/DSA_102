...
- Rabin-Karp: Use rolling hash to skip non-matching positions
...
**Why These Matter:**
- Rabin-Karp: Practical string matching, plagiarism detection, DNA sequence searching
...
8. **Implement rolling hash** and Rabin-Karp string matching
...
| Practice Day 4-5 problems | 4 | Implement hash table, Rabin-Karp, collision tests |
...
4. **TimSort (Python, Java object arrays) is a stable hybrid:** It exploits natural runs, merges them strategically, and uses insertion sort on small runs. Introsort, not TimSort, is the quicksort/heapsort hybrid used by C++ std::sort.
...
8. **Rabin-Karp is O(n) on average but O(n·m) worst-case** (hash collisions). Still practical because collisions are rare with good hash.
...
- [ ] Implement Rabin-Karp rolling hash pattern matching
