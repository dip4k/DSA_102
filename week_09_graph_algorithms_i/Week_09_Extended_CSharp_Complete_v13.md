...
#### 🔴 C# Engineering Notes
- 🔴 **CRITICAL:** Union-Find should use both path compression and union by rank/size. Without both optimizations, operations are typically O(log n) in well-shaped forests and can degrade badly under poor implementations; with both, amortized time is O(α(n)), which is effectively constant in practice.
