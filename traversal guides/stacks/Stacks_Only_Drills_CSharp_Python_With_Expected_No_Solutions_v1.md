# 🧱 Stacks-only Drills (No Solutions) — Python + C#

## 🐍 Option A: Python driver
Create `stacks_drills.py` and run:
```bash
python stacks_drills.py
python stacks_drills.py S7
```
...
def run(drill_id="ALL"):
    ...
    if drill_id not in tests:
        raise ValueError("Unknown id. Use ALL or S1..S11 or P1..P2 or I2")
...
Create a project:
```bash
dotnet new console -n StacksOnlyDrills
cd StacksOnlyDrills
```
...
public sealed class Runner
{
    public void Run(string id)
    {
        ...
        if (!tests.TryGetValue(id, out var one))
            throw new Exception("Unknown id. Use ALL or S1..S11 or P1..P2 or I2");
