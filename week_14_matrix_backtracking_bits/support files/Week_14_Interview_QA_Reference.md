...
*   **Verbal Answer**:
    ...
    1.  First, reverse each row of the matrix.
    2.  Second, transpose the matrix symmetrically along its main diagonal (M[i][j] <=ftrightarrow M[j][i] where j > i).
    This sequence composition of swaps rotates the coordinates cleanly with O(1) auxiliary space and avoids complex, error-prone projection coordinate formulas."
*   **Complexity**: Time: O(N^2) | Space: O(1).

---

#### Q2. How do you search for an element in an M x N matrix where both the rows and columns are sorted?
*   **Verbal Answer**:
    "We can solve this efficiently in linear O(M + N) time using a **Staircase Search** (often called the saddlepoint search).
    Starting at the top-right corner `(row = 0, col = N-1)`:
    *   If our target is smaller than the current element, we can safely eliminate the entire current column and move **left** (`col--`).
    *   If our target is larger, we can safely eliminate the current row and move **down** (`row++`).
    Repeat this process until we either find the target or move outside the grid boundaries. This avoids redundant work and outperforms standard binary searches on individual rows (O(M log N))."
*   **Complexity**: Time: O(M + N) | Space: O(1).

---

#### Q3. How can we set entire rows/columns to zero in-place while keeping our space complexity at O(1)?
*   **Verbal Answer**:
    "A naive approach uses extra arrays of size M and N to mark which rows and columns need zeroing, which takes O(M + N) space.
    To achieve O(1) space, we can reuse the first row and column of the matrix itself as our markers in-place.
    We first scan the first row and column to check if they contain any zeros and set corresponding flags (`firstRowZero`, `firstColZero`).
    Then, we scan the rest of the matrix `M[1...rows][1...cols]`. If a cell is `0`, we set its row and column headers in the first row and column to `0`: `M[i][0] = 0` and `M[0][j] = 0`.
    Finally, we scan the matrix again to zero cells based on these markers, and then zero the first row and column if our flags were set."
*   **Complexity**: Time: O(M * N) | Space: O(1).
...
*   **Verbal Answer**:
    "The bit operation `n & (n - 1)` clears the rightmost set bit in an integer. If the result is `0`, it indicates that the number has at most one set bit.
...
*   **Verbal Answer**:
    ...
    **Brian Kernighan's algorithm** is faster because it executes exactly once per active set bit. It uses the property that subtracting 1 from a number flips all bits starting from its rightmost set bit.
    Therefore, performing n \ {AND}\ (n-1) clears the lowest set bit in exactly one cycle, skipping all intermediate zeros."
*   **Complexity**: Time: O({set bits}) steps.
...
*   **Verbal Answer**:
    ...
    B x x == 1 +/-od M
    This inverse exists if and only if B and M are coprime (gcd(B, M) = 1).
    If M is prime, we can use **Fermat's Little Theorem** to calculate the inverse in logarithmic time:
    A^{-1} == A^{M-2} +/-od M
    If M is not prime, we must use the **Extended Euclidean Algorithm** to calculate his inverse."
...
*   **Verbal Answer**:
    ...
    In high-precision computing, we can partition calculations on extremely large numbers by running them in parallel modulo smaller, coprime moduli. We then reconstruct the final high-precision result using the CRT formula. This speeds up execution because operations on smaller integer types run directly in hardware registers, avoiding the high cost of arbitrary-precision software structures."
...
*   **Verbal Answer**:
    ...
    *   **KMP (Knuth-Morris-Pratt)**: Matches a **single** pattern P of length M against a streaming text T of length N. It uses a precomputed fallback table (LPS table).
    *   **Aho-Corasick**: Matches **multiple** patterns simultaneously. It compiles a dictionary of words into a single prefix tree (Trie), then adds failure links to connect mismatch points to the longest valid suffix traversed so far.
    This allows Aho-Corasick to scan a streaming text for thousands of words simultaneously in a single linear pass."
