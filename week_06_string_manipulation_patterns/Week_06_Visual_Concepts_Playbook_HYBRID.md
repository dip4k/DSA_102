...
- Day 5: Advanced Pattern Matching (Rolling hash, Rabin-Karp)
...
Why is Rabin-Karp O(n+m) expected but O(nm) worst-case? When would worst-case occur?
...
**Q3: Real-World Application**
Turnitin plagiarism detection searches 100 million documents for plagiarized substrings. Would you use Rabin-Karp for each pattern, or Aho-Corasick (multi-pattern matching)? Why?
...
| Rabin-Karp matching | O(n+m) | O(n+m) | O(nm)* | O(1) | Multiple patterns |
