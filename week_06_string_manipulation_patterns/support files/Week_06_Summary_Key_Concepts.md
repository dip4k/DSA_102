...
**Day 5: Advanced Pattern Matching (Optional)** — Scale pattern matching to billions using rolling hash. Rabin-Karp computes hashes incrementally in O(1), reducing matching to O(n+m) expected time. Mental model: "Rolling polynomial for scale."
...
### Day 5: Advanced Pattern Matching — Rolling Hash & Rabin-Karp (Optional)
...
- **Rabin-Karp:** O(n+m) avg, simple code, multi-pattern friendly
...
| **Rabin-Karp** | **O(n+m)** | **O(n+m)** | O(nm) collisions | **O(1)** | **Multiple patterns** |
...
| Repeated DNA sequences | Rabin-Karp rolling hash | O(n) | O(1) | 🟡 Medium |
...
### Misconception 6: Hash Collisions Ruin Rabin-Karp
...
**False Reality:** Rabin-Karp is surprisingly simple—just rolling polynomial arithmetic. KMP is harder but guarantees worst-case O(n+m). Choose based on problem.
