...
**Q2 (🟢 Easy):** What's the Rabin-Karp algorithm? Give a one-sentence summary.
- **Follow-up:** When is hash match NOT a string match?
- **Follow-up:** Why do you verify after hash match?

**Q3 (🟡 Medium):** Implement Rabin-Karp pattern matching for pattern P in text T.
...
**Q5 (🔴 Hard):** Compare Rabin-Karp (O(n+m) avg), KMP (O(n+m) worst), Boyer-Moore (O(n/m) best). Which would you use for each scenario?
...
- **Follow-up:** Can you run Rabin-Karp 100 times or is Aho-Corasick better?
