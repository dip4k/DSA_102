...
This guide brings together the **most frequently asked Low-Level Design (LLD), Object-Oriented Design (OOD), and Machine Coding problems** across top global and product-driven tech companies (Google, Microsoft, Amazon, Meta, Apple, Netflix, Uber, Stripe, Atlassian, Bloomberg, Swiggy, Flipkart, PhonePe, Razorpay).

* 🏢 **For complete company-by-company breakdown**, see the **[Top 30 Product Companies LLD Interview Directory & Prep Guide](./Top_30_Product_Companies_LLD_Interview_Guide.md)**.
* 🧠 **For interview pacing, checklists, and latency numbers**, see **[10. Interview Quick Reference, Decision Trees & Checklists](../00_Core_Concepts/10_Interview_Quick_Reference_and_Checklists.md)**.

These questions are **strictly language-agnostic** in architectural principle and implemented with production-grade **C# 12 / .NET 8**, balancing **clean SOLID abstractions** with **optimal data structures ($O(1)$ / $O(\log N)$)** under interview pressure.
...
| :---: | :--- | :---: | :--- | :--- |
| **146** | [LRU Cache](https://leetcode.com/problems/lru-cache/) · [*(C# Solution)*](../03_Tier3_DSA_Screening/26_LRU_and_LFU_Cache.md) | Medium | HashMap + Doubly Linked List (O(1) Get/Put) | Universal (Google, Amazon, Meta, Microsoft) |
| **460** | [LFU Cache](https://leetcode.com/problems/lfu-cache/) · [*(C# Solution)*](../03_Tier3_DSA_Screening/26_LRU_and_LFU_Cache.md) | Hard | Two HashMaps + Freq-ordered Doubly Linked Lists | Amazon, Microsoft, Apple, Uber, Citadel |
| **588** | [Design In-Memory File System](https://leetcode.com/problems/design-in-memory-file-system/) · [*(C# Solution)*](../02_Tier2_Classic_LLD/17_In_Memory_File_System.md) | Hard | Trie (Prefix Tree) / Composite Pattern + RWLock | Amazon, Uber, Airbnb, Google |
| **642** | [Design Search Autocomplete System](https://leetcode.com/problems/design-search-autocomplete-system/) · [*(C# Solution)*](../03_Tier3_DSA_Screening/29_Search_Autocomplete_System.md) | Hard | Trie with Top-K PriorityQueue / Min-Heap | Google, Amazon, Microsoft, Meta |
| **2034**| [Stock Price Fluctuation](https://leetcode.com/problems/stock-price-fluctuation/) · [*(C# Solution)*](../04_Tier4_Senior_Backend/35_Stock_Exchange_Matching_Engine.md) | Medium | HashMap (timestamps) + Ordered Set / Two Heaps | Goldman Sachs, Google, Citadel |
| **2353**| [Design a Food Rating System](https://leetcode.com/problems/design-a-food-rating-system/) · [*(C# Solution)*](../02_Tier2_Classic_LLD/16_Food_Delivery_System.md) | Medium | Two HashMaps + Balanced BST / SortedSet | DoorDash, Uber, Amazon |
...
| **981** | [Time Based Key-Value Store](https://leetcode.com/problems/time-based-key-value-store/) · [*(C# Solution)*](../04_Tier4_Senior_Backend/33_Transactional_Key_Value_Store.md) | Medium | HashMap of Sorted Lists + Binary Search (`Upper_Bound`) | Google, Netflix, Amazon, Apple |
...
| **1166**| [Design File System](https://leetcode.com/problems/design-file-system/) · [*(C# Solution)*](../02_Tier2_Classic_LLD/17_In_Memory_File_System.md) | Medium | Trie / Prefix HashMap | Airbnb, Google |
...
| :---: | :--- | :---: | :--- | :--- |
| **348** | [Design Tic-Tac-Toe](https://leetcode.com/problems/design-tic-tac-toe/) · [*(C# Solution)*](../02_Tier2_Classic_LLD/18_Board_Game_Engine.md) | Medium | O(1) Row/Col/Diagonal Counters (No full board scan) | Amazon, Microsoft, Meta |
| N/A | [Snake and Ladder Game](../02_Tier2_Classic_LLD/23_Snake_and_Ladder.md) | Medium | Strategy (Dice), Dynamic Board, Cycle Detection | Flipkart, Amazon, Swiggy, Zepto |
...
| **2241**| [Design an ATM Machine](https://leetcode.com/problems/design-an-atm-machine/) · [*(C# Solution)*](../02_Tier2_Classic_LLD/21_ATM_System.md) | Medium | State Machine + Banknote Dispenser Backtracking | Microsoft, Atlassian, Oracle |
...
| :---: | :--- | :---: | :--- | :--- |
| **355** | [Design Twitter / Social Feed](https://leetcode.com/problems/design-twitter/) · [*(C# Solution)*](../03_Tier3_DSA_Screening/30_Social_Media_Feed_Twitter.md) | Medium | PriorityQueue K-way merge + Fanout Hybrid | Twitter/X, Meta, Amazon |
...
| N/A | [Design Rate Limiter](../04_Tier4_Senior_Backend/31_Rate_Limiter.md) | Medium/Hard | Token Bucket, Leaky Bucket, Sliding Window Log | Stripe, Uber, Atlassian |
| N/A | [Design URL Shortener (TinyURL)](../04_Tier4_Senior_Backend/37_URL_Shortener.md) | Medium | Base62 Encoding + Distributed Range Allocator | Google, Microsoft, Amazon |
...
1. **[Ride-Sharing / Cab Booking System (Uber / Ola / Grab)](../02_Tier2_Classic_LLD/15_Ride_Hailing_System.md)**
...
2. **[Food Delivery Service (Swiggy / Zomato / DoorDash)](../02_Tier2_Classic_LLD/16_Food_Delivery_System.md)**
...
3. **[Movie Ticket Booking System (BookMyShow / Ticketmaster)](../02_Tier2_Classic_LLD/09_Movie_Booking.md)**
...
5. **[Snake and Ladder Game (Flipkart / Amazon / Swiggy)](../02_Tier2_Classic_LLD/23_Snake_and_Ladder.md)**
   * *Key Entities:* `Board`, `Cell`, `Player`, `Dice`, `IDiceStrategy`, `ISpecialCell`, `Snake`, `Ladder`.
   * *Patterns:* **Strategy** (Normal vs Crooked dice), **Cycle Detection** (3-color DFS to prevent infinite game loops), **Observer** (Leaderboard / rank alerts).
   * *Interview Focus:* Clean OOP separation, extensible board size $N \times N$, boundary overshoot rules.
6. **[Cricbuzz / Live Cricket Scoreboard & Commentary Engine](../02_Tier2_Classic_LLD/24_Cricbuzz_Cricket_Scoreboard.md)**
   * *Key Entities:* `Match`, `Innings`, `Over`, `Ball`, `BallType`, `Player` (`BatsmanStats`, `BowlerStats`), `Team`.
   * *Patterns:* **Domain Service** (`StrikeRotator`), **Observer Pattern** (Commentary, Push notifications, UI scorecards).
   * *Interview Focus:* Strict 6-legal-ball over accounting, extras (wides/no-balls) without incrementing ball count, strike change on odd runs and over ends.
7. **[Coupon & Discount Rule Engine (Amazon / Flipkart / Swiggy)](../02_Tier2_Classic_LLD/25_Coupon_and_Discount_Engine.md)**
   * *Key Entities:* `Cart`, `CartItem`, `Product`, `DiscountRule`, `Coupon`, `IRuleSpecification`, `IDiscountStrategy`.
   * *Patterns:* **Composite Specification** (`And`, `Or`, `MinCartValue`), **Strategy** (Percentage with caps, Flat, Buy X Get Y), **Pipeline**.
   * *Interview Focus:* Stacking order optimization, mutual exclusivity, atomic coupon quota consumption (`Interlocked.CompareExchange`).

### 2.2 Financial & Infrastructure LLD
8. **[Digital Wallet & Ledger System (PhonePe / Razorpay / Stripe)](../04_Tier4_Senior_Backend/34_Digital_Wallet_Ledger.md)**
...
9. **[In-Memory Transactional Key-Value Store (Redis-Lite)](../04_Tier4_Senior_Backend/33_Transactional_Key_Value_Store.md)**
...
10. **[Task Management & Sprint Planner (Jira Lite)](../02_Tier2_Classic_LLD/14_Task_Management_System.md)**
...
11. **[Stock Exchange / Order Matching Engine (Zerodha / Nasdaq Lite)](../04_Tier4_Senior_Backend/35_Stock_Exchange_Matching_Engine.md)**
...
12. **[Chess Game / Board Game Engine (Tic-Tac-Toe & Chess Lite)](../02_Tier2_Classic_LLD/18_Board_Game_Engine.md)**
   * *Key Entities:* `Board`, `Cell`, `Piece`, `Move`, `Player`, `RulesEngine`.
   * *Patterns:* **Strategy** (Move validation for Pawn, Rook, Knight; Special rules like Castling/En Passant), **Command** (Undo/Redo).
13. **[High-Throughput URL Shortener (TinyURL)](../04_Tier4_Senior_Backend/37_URL_Shortener.md)**
   * *Key Entities:* `UrlMapping`, `Base62Encoder`, `AtomicRangeTokenGenerator`, `AsyncAnalyticsService`.
   * *Patterns:* **Bijective Base62**, **Range Allocation Lease**, **Cache-Aside**, **Non-blocking Channels**.
   * *Interview Focus:* Zero-collision ID generation without DB round-trips, sub-5ms redirect caching.
